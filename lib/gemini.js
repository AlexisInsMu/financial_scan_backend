    ...
    productos: {
      ...
      items: {
        ...
        properties: {
          ...
          categoria: {
            type: SchemaType.STRING,
            description: "Debe ser: comida,servicio, alimentos,transporte,hormiga,entretenimiento,salud,compras u otros"
          },
...
const analisisGeneralSchema = {
  ...
  properties: {
    resumen_ejecutivo: {
      type: SchemaType.STRING, description: `Análisis financiero como contexto, genera únicamente un resumen ejecutivo ultra corto.
Formato obligatorio:
        - Máximo 200 caracteres
- Debe caber visualmente en la pantalla de un teléfono en vertical
- Cada párrafo de 2 a 3 líneas máximo
- Lectura rápida en menos de 20 segundos
  - Español claro, cercano y motivador
    - Sin tecnicismos ni redundancia

Debe explicar:
1. El estado actual de la salud financiera
2. El hábito que más impacta positiva o negativamente el score
3. Si el usuario ya está listo para iniciar inversiones pequeñas o si primero debe mejorar control y estabilidad financiera

Regla según score:
- Score alto: sugerir inversiones conservadoras
  - Score medio: sugerir micro inversiones si mantiene consistencia 30 días
    - Score bajo: priorizar reducir gastos hormiga y construir colchón

Cerrar con una siguiente acción clara en una frase.` },
diagnostico: {
  type: SchemaType.OBJECT,
    properties: {
    nivel_actual: {
  ...
  required: ["nivel_actual", "fortalezas", "riesgos", "patrones_detectados"]
},
insights_clave: {
  type: SchemaType.ARRAY,
    items: {
    ...
    required: ["titulo", "explicacion", "impacto_estimado_mxn_mes"]
  }
},
metas_recomendadas: {
  type: SchemaType.OBJECT,
  ...
  required: ["meta_gasto_hormiga_pct", "meta_tickets_semanales", "ahorro_potencial_mxn_mes"]
},
  },

required: ["resumen_ejecutivo", "diagnostico", "insights_clave", "metas_recomendadas"]
};
...
- En cada elemento de insights_clave incluye SIEMPRE: titulo, explicacion e impacto_estimado_mxn_mes.
- impacto_estimado_mxn_mes debe ser numérico en MXN (si no se puede estimar con precisión, usa 0).
